      ...
      try {
        ...
        try {
          ...
          const membersResponse = await axios.get(
            `/api/project-members?projectDocumentId=${projectId}&projectIdNumber=${projectResponse.data.project?.id}`
          );
          if (
            membersResponse.data.success &&
            membersResponse.data.projectMembers
          ) {
            console.log(
            ...
            const membersWithUserInfo = await Promise.all(
              membersResponse.data.projectMembers.map(async (member: any) => {
                try {
                  ...
                  console.error(
                    `Error fetching user ${member.user_id_in_project}:`,
                    userError
                  );
                  return {
                    ...member,
            ...
            if (currentUser && projectResponse.data.project) {
              ...
              if (isProjectCreator) {
                ...
                const userMembership = membersWithUserInfo.find(
                  (member) =>
                    (member.userInfo?.id || member.user_id_in_project) ===
                    currentUserId
                );
        ...
        try {
          ...
          if (tasksResponse.data.success && tasksResponse.data.tasks) {
            ...
            if (currentUserId) {
              ...
              console.log(
                `Found ${userTasks.length} tasks for current user (${currentUserId})`
              );
              console.log(
                `Found ${otherUserTasks.length} tasks for other users`
              );
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (tasksResponse.data.success && tasksResponse.data.tasks) {
          ...
          if (currentUserId) {
            const userTasks = allTasks.filter(
              (task: Task) => task.assigned_to_user_ids_number === currentUserId
            );
            const otherUserTasks = allTasks.filter(
              (task: Task) => task.assigned_to_user_ids_number !== currentUserId
            );
  ...
  const handleRemoveMember = async (memberId: number, memberName: string) => {
    ...
    try {
      const response = await axios.delete(
        `/api/project-members?memberId=${memberId}`
      );
  ...
  const renderTaskCard = (task: Task, isMyTask: boolean = false) => {
    ...
    const getAssigneeInfo = () => {
      ...
      if (assignedMember) {
        return {
          name:
            assignedMember.userInfo?.username ||
            `User ${assignedMember.user_id_in_project}`,
    ...
    const cardBg =
      task.task_status.toLowerCase() === "turn in" ? "bg-green-50" : "bg-white";
    const borderColor =
      task.task_status.toLowerCase() === "turn in"
        ? "border-green-200"
        : "border-gray-200";
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex justify-between items-center">
            <div className="text-gray-700">
              <span className="font-medium">ผู้รับผิดชอบงาน:</span>{" "}
              {assigneeInfo.name}
            </div>
            <div className="text-gray-600">
              <span className="font-medium">ระยะเวลางาน</span>{" "}
          ...
          <div className="flex justify-between items-center">
            ...
            <div
              className={`px-3 py-1 rounded-full text-sm font-medium ${statusConfig.statusBg}`}
            >
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-6">
            โปรเจ็กต์ที่คุณกำลังมองหาอาจถูกลบหรือย้ายไปแล้ว
          </p>
          <a
            href="/overview"
            className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-medium transition-colors"
        ...
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg mx-auto transform transition-all animate-in slide-in-from-bottom-4 duration-300">
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-100">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-xl font-bold text-gray-900">
                  เพิ่มสมาชิกโปรเจ็กต์
                </h3>
                <p className="text-sm text-gray-500">
                  เชิญสมาชิกใหม่เข้าร่วมโปรเจ็กต์
                </p>
              </div>
            </div>
            <button
                ...
                <div className="text-center py-12 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
                  <div className="flex flex-col items-center">
                    ...
                    <p className="text-sm text-gray-500 max-w-sm">
                      ผู้ใช้ทั้งหมดเป็นสมาชิกของโปรเจ็กต์นี้แล้ว
                      หรืออาจไม่มีผู้ใช้อื่นในระบบ
                    </p>
                  </div>
                </div>
              ) : (
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      {availableUsers.map((user) => (
                        <option key={user.id} value={user.id} className="py-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { value: "Member", label: "สมาชิก", desc: "สมาชิกทั่วไป" },
                  {
                    value: "Leader",
                    label: "หัวหน้าทีม",
                    desc: "ควบคุมโปรเจ็กต์",
                  },
                ].map((roleOption) => (
                  <div key={roleOption.value} className="col-span-1">
                    <label
                      className={`relative flex items-start p-4 border-2 rounded-xl cursor-pointer transition-all hover:bg-gray-50 ${
                      ...
                      >
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-gray-500">
                            {member.role_in_project} • เข้าร่วมเมื่อ{" "}
