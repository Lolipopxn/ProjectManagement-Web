// Task Status Icons Component
// ไฟล์นี้จัดการ icons สำหรับสถานะ task
...
export const TaskStatusIcon: React.FC<TaskStatusIconProps> = ({ status, className = "w-6 h-6" }) => {
  switch (status.toLowerCase()) {
    case 'completed':
      return (
        <svg className={`${className} text-green-700`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    case 'turn in':
      return (
        <svg className={`${className} text-blue-700`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    case 'pending':
      return (
        <svg className={`${className} text-yellow-700`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
